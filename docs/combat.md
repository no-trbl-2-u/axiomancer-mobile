...
| `choosing_skill` | Horizontal scroll of skill cards filtered by current stance; greys out skills with `wrong-stance` or `insufficient-mana`. Skills come from a fixture ([Q3](../specs/04-combat-screen-wiring.md) = A) — search for `TODO: replace with engine skills` to find the swap site once engine Spec 04 ships. | Tap an enabled skill to spend mana and resolve. | `vm.skillPicker` |
