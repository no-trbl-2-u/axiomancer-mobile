...
- [ ] Phase 137 — Full character update deltas for equip changes. Filed by T direct steering 2026-06-17 as the next phase after dev loot rarity controls. Update the "updated stats" equip/swap/unequip section into a complete changed-only character-update preview: render every changed character-facing value caused by equip, unequip, or swap; green text for increases/new item values, red text for decreases/old item values; passive effects use red for old/removed item effects and green for new/gained item effects. Brief: `plan/phases/phase_137_full_character_update_deltas.md`. Verification: equipDelta presenter + EquipDeltaPanel + inventory e2e tests + `npm run typecheck` + `npm run verify`. Source: T Telegram steering 2026-06-17.
