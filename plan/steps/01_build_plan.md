...
- [ ] Phase 134 — Mechanics 0.22.0 mobile catch-up. Filed by T direct steering 2026-06-17 after `axiomancer-mechanics@0.22.0` was published. Bump mobile from 0.21.x to 0.22.0 and drain integration fallout from the affix release: structured `Equipment` affix fields (`prefixId`, `suffixId`, `prefixName`, `suffixName`), rarity-affix defaults in `dropItem`, trimmed equipment templates, and curated affixed variants. Do not simulate affixes locally and do not parse affix truth from item names. Brief: `plan/phases/phase_134_mechanics_0_22_mobile_catchup.md`. Verification: package version check + inventory/cache/dev-item focused tests + `npm run typecheck` + `npm run verify`. Source: T Telegram steering 2026-06-17.
