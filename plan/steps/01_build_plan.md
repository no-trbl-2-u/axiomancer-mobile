...
- [ ] Phase 93 — Combat UI/UX design overhaul. Promoted via `/oversight` 2026-05-29 (42nd call) from PHASE_CANDIDATES `[score 8.5]`. CRITIQUE.md [HIGH] /combat row + playtest findings F02–F06 (encounter jargon, battle-log ability names, LET phase numbers, CRUCIBLE symbols) all point at the same fundamental UX gap. Two-stage phase: (a) request design bundle via `claude.ai/design` covering combat-modal numerics + iconography legibility + stance-card layout shrink-to-fit (PHASE_CANDIDATES [score 3.5] F07) — produce design prompt under `design/combat-overhaul-prompt.txt`; (b) port the bundle once it lands. /march should ship stage (a) — the prompt draft — and then defer stage (b) until the bundle is vendored at `design/handoff-<date>/`. Stays gated similarly to how Phase 73 (LevelUp) was gated on the bundle landing.

- [ ] Phase 94 — Title screen + new-player onboarding. Promoted via `/oversight` 2026-05-29 (42nd call) from CRITIQUE.md [HIGH] "No title screen or onboarding for new players" (deep-playtest [F01]). Game currently loads directly to WILDS with zero context for vocabulary (PILGRIM, VITAE, LEAGUES) or goals. Two-stage like Phase 93: (a) request design bundle via `claude.ai/design` covering title card + minimal vocabulary primer (1–2 surfaces; stays diegetic — no tutorial overlay arrows) — produce design prompt under `design/title-screen-prompt.txt`; (b) port once bundle lands. /march should ship stage (a) — the prompt draft — and defer stage (b) until the bundle is vendored.

- [ ] Phase 95 — FLEE narrative feedback + morale UI surface. Promoted via `/oversight` 2026-05-29 (42nd call) from PHASE_CANDIDATES `[score 5.0]` (playtest F03) + CRITIQUE.md [HIGH] /encounter row. Two-piece, single phase: (1) Code-side narrative beat — after FLEE resolves, surface a one-line prose beat in the same register as death/victory copy (e.g. `'You withdrew from the foe.'`) via the existing AftermathBanner pattern or a paced toast; presenter-only change. (2) Morale UI surface — expose `state.player.morale` on the exploration card OR SELF tab as a small chip with `-ii` deltas animating when FLEE deducts. Code-side first (the chip), design-routed for the bar visual if a design pass is needed later. Verify gate: `npm run verify`.

- [ ] Phase 96 — Death-screen presenter fixes. Promoted via `/oversight` 2026-05-29 (42nd call) from PHASE_CANDIDATES `[score 4.5]` (playtest F09, F10) + CRITIQUE.md [MED] /death row. Two concrete presenter fixes in `state/presenters/aftermath.engine.ts` (or wherever the defeat LEDGER selector lives): (1) `encountersSurvived` counter logic — dying in an encounter must count that encounter as `0` survived, not `1`; (2) `deepestNode` field — resolve internal node id (e.g. `'fv-14'`) to the human-readable map-layout label (e.g. `'Tide Pool'`) via the existing exploration-map fixture. +2 hermetic cases pinning both. Verify gate: `npm run verify`.

- [ ] Phase 97 — Playwright multi-screen flow scripts (NEEDS_HUMAN_ATTENTION Option A). Promoted via `/oversight` 2026-05-29 (42nd call) from NEEDS_HUMAN_ATTENTION [tooling] testing-gap row (Open since 2026-05-21). Extend `scripts/smoke-screens.mjs` with interactive flow scripts under `scripts/flows/`: (1) `encounter-stays-in-modal.mjs` — load exploration → tap encounter node → tap FIGHT → assert encounter-modal-overlay still in DOM, tab bar `display: none`, combat UI rendered inside the modal; (2) `stance-advances-phase.mjs` — start combat → tap stance card → assert PhaseStack `choosing_action` row is `state="current"`; (3) `action-resolves-round.mjs` — pick stance → pick ATTACK → assert ResolvePanel mounted + log entry added. Documents `npm run smoke:flows` script in `package.json`. Would have caught Phase 62/63 multi-screen bugs. Option B (Jest multi-screen integration tests) ships in parallel as an `/iterate` stream per the bias note at top of `plan/AUDIT.md` (no phase needed — set this oversight call).
...
- [x] Phase 91 — Battle log two-line format. Promoted via
      ...
      after the build-plan queue emptied. Shipped 2026-05-27 as
      part of the Combat UX Boards design implementation: severity
      rail (4px colored bar per entry), log legend bar, and scroll
      hint. The visual severity-rail pattern satisfies the brief's
      core intent (connect choice to outcome via color-coded
      structure). Remaining two-line presenter splitting (choice
      line + payload chip) filed as a follow-up candidate.
      See `plan/phases/phase_91_battle_log_two_line_format.md`.
