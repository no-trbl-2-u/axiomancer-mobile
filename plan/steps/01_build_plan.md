...
- [ ] Phase 127 — Combat skill confirmation overlay and deterministic skill result UI. Filed by T direct steering 2026-06-15. Because player skills always hit and have static mechanics-owned damage/effects, remove the dice-roll tracker from skill use. Tapping a skill opens a detail overlay showing skill name, cost, deterministic damage/effects/statuses, keywords, and relevant engine-owned notes; `Confirm` commits the skill and `Cancel` returns without side effects. When mechanics Phase 150 exposes enemy skill-answer event detail, render that answer distinctly; otherwise document the exact contract blocker. Brief: `plan/phases/phase_127_combat_skill_confirm_overlay_deterministic_results.md`. Verification: focused combat skill overlay Jest + `npm run typecheck` + `npm run verify` + visual-smoke evidence or exact blocker.
