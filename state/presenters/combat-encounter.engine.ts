...
export function faceStats(card: CombatCard, skill?: Skill): CombatCardFaceVM {
    ...
    switch (c.kind) {
        case 'dot': return { ...base, kind: 'dot', keyword: kw, heroText: `${c.total}`, heroSub: `over ${c.turns} turns`, freeHeroText: free, freeHeroSub: null, verbLine: 'foe loses HP each turn', powerRail: c.keyword ?? 'DoT', readDependent: false, inert: false, guardBase: null };
        ...
        case 'vulnerable': return { ...base, kind: 'vulnerable', keyword: kw, heroText: `+${c.vulnPct}%`, heroSub: `dmg taken · ${c.turns} turns`, freeHeroText: free, freeHeroSub: null, verbLine: 'foe takes more damage', powerRail: c.keyword ?? 'Vulnerable', readDependent: false, inert: false, guardBase: null };
...
function detailCore(card: CombatCard, skill?: Skill): DetailCore {
    ...
    switch (c.kind) {
        case 'dot': return { subtitle: `${Title} the enemy — damage over time.`, metaChip, outcomeLine: `${c.perTurn} HP/turn for ${c.turns} turns — ${c.total} HP total.`, outcomeStats: [{ label: 'PER TURN', value: `${c.perTurn}` }, { label: 'TURNS', value: `${c.turns}` }, { label: 'TOTAL', value: `${c.total}` }], stacksText: c.stacks ? 'Stacks up to 10× — re-applying piles on more.' : null, freeLine: `◇ FREE (no die): deal ${free} HP now. No ${Title} lands without a die.`, powerLine: `◆ WITH A DIE: apply ${Title} — ${c.perTurn} HP/turn for ${c.turns} turns (${c.total} total), plus a small hit.`, readNote: `The read scales only the small strike, not the ${Title} — ${c.total} is exact.`, mathLine: `${c.perTurn}/turn = ${c.dpr} base × ${c.intensity} intensity · ${c.turns} turns · ${c.total} HP total${c.stacks ? ' · stacks to 10×' : ''}.`, keywords };
        case 'stun': return { subtitle: `${Title} the enemy — it loses its turns.`, metaChip, outcomeLine: `The enemy skips its next ${c.skips} turns — ${c.skips} telegraphed attacks it never makes.`, outcomeStats: [{ label: 'SKIPS', value: `${c.skips} turns` }], stacksText: null, freeLine: `◇ FREE (no die): deal ${free} HP now. No ${Title} without a die.`, powerLine: `◆ WITH A DIE: ${Title} — the foe skips its next ${c.skips} actions, plus a small hit.`, readNote: `The read scales only the small strike, not the ${Title} — skip ${c.skips} is exact.`, mathLine: `skip ${c.skips}t = ${Title.toLowerCase()} duration ${c.skips} (each turn it would act is cancelled).`, keywords };
        case 'weaken': return { subtitle: `${Title} the enemy — its attacks hit softer.`, metaChip, outcomeLine: `Weaken its next hit.`, outcomeStats: [], stacksText: null, freeLine: `◇ FREE (no die): deal ${free} HP now. No ${Title} without a die.`, powerLine: `◆ WITH A DIE: apply ${Title} — the foe's next hit is weakened, plus a small hit.`, readNote: `${Title} weakens the enemy's hit; pile on different controls to deny the turn outright.`, mathLine: `${Title} lowers the enemy's attack roll; combined controls past a threshold deny the turn (exact amount is engine-tuned).`, keywords };
        ...
        case 'vulnerable': { const cap = Math.round((VULNERABLE_MAX_MULT - 1) * 100); return { subtitle: `${Title} the enemy — it takes more damage.`, metaChip, outcomeLine: `+${c.vulnPct}% damage taken for ${c.turns} turns.`, outcomeStats: [{ label: 'DMG TAKEN', value: `+${c.vulnPct}%` }, { label: 'TURNS', value: `${c.turns}` }], stacksText: `Re-applying refreshes the ${c.turns}-turn window; stacking different Vulnerable sources climbs toward the +${cap}% cap.`, freeLine: `◇ FREE (no die): deal ${free} HP now. No ${Title} lands without a die.`, powerLine: `◆ WITH A DIE: apply ${Title} — +${c.vulnPct}% damage taken for ${c.turns} turns, plus a small hit.`, readNote: `The read scales only the small strike, not ${Title} — +${c.vulnPct}% is exact.`, mathLine: `+${c.vulnPct}% = (damageTakenMult − 1) × 100; combined Vulnerable caps at +${cap}%.`, keywords }; }
        case 'thorns': return { subtitle: `${Title} — attackers take damage back.`, metaChip, outcomeLine: `Gain ${Title}: reflect ${c.reflectN} damage to the enemy each time it hits you, for ${c.turns} turns.`, outcomeStats: [{ label: 'REFLECT', value: `${c.reflectN}` }, { label: 'TURNS', value: `${c.turns}` }], stacksText: c.intensity > 1 || (lookupEffect(c.ce?.effectId ?? '')?.stacking === 'intensity') ? 'Stacks — re-applying reflects more.' : null, freeLine: `◇ FREE (no die): still gain ${Title} — Reflect ${c.reflectN} for ${c.turns} turns (a self-buff lands without a die).`, powerLine: `◆ WITH A DIE: gain ${Title} (Reflect ${c.reflectN}) plus a small hit.`, readNote: 'Your reflect takes no read — Reflect is exact; the read scales only the small strike.', mathLine: `Reflect ${c.reflectN} returned per enemy hit, ${c.turns} turns; stacking raises the reflect.`, keywords };
        ...
        case 'rupture': return { subtitle: "Rupture — detonate the foe's damage-over-time.", metaChip, outcomeLine: `Consumes ALL the enemy's stacked DoT and deals it instantly as a burst (capped at ${RUPTURE_BURST_CAP}).`, outcomeStats: [{ label: 'BURST', value: 'live total' }, { label: 'CAP', value: `${RUPTURE_BURST_CAP}` }], stacksText: null, freeLine: '◇ FREE (no die): chip a sliver of HP — Rupture needs a die.', powerLine: `◆ WITH A DIE: detonate the foe's pending DoT for a burst (up to ${RUPTURE_BURST_CAP}); the read scales it.`, readNote: 'Stack DoT first — the burst equals the pending DoT, so it has no fixed number until you fire it.', mathLine: `Burst = pending DoT × read (capped ${RUPTURE_BURST_CAP}) — pending DoT is live, so no fixed number (real-units-or-no-number).`, keywords };
        case 'inert':
        default: return { subtitle: `${Title || 'Effect'} — minor right now.`, metaChip, outcomeLine: `${Title || 'This effect'} — minor right now. The live engine does not read it yet, so it is shown without a number (no fabricated value).`, outcomeStats: [], stacksText: null, freeLine: `◇ FREE (no die): deal ${free} HP now.`, powerLine: `◆ WITH A DIE: apply ${Title || 'the effect'}, plus a small hit.`, readNote: 'The read scales only the small strike.', mathLine: `${Title || 'This effect'} is not read by the live HP engine yet — no number shown.`, keywords };
    }
}
